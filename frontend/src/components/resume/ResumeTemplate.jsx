...
/**
 * ResumeTemplate - Standalone resume template component
 * Accepts resume and customization as props instead of using context
 * 
 * @param {Object} props
 * @param {Object} props.resume - The resume data object
 * @param {Object} props.customization - Optional customization settings
 */
export default function ResumeTemplate({ resume, customization: customizationProp }) {
  // Merge with default customization
  const customization = useMemo(() => ({
    ...defaultCustomization,
    ...customizationProp,
    colors: { ...defaultCustomization.colors, ...customizationProp?.colors },
    typography: { ...defaultCustomization.typography, ...customizationProp?.typography },
    spacing: { ...defaultCustomization.spacing, ...customizationProp?.spacing },
    layout: { ...defaultCustomization.layout, ...customizationProp?.layout },
    sectionStyles: { ...defaultCustomization.sectionStyles, ...customizationProp?.sectionStyles },
    options: { ...defaultCustomization.options, ...customizationProp?.options },
  }), [customizationProp]);

  const sections = useMemo(() => resume?.sections || [], [resume?.sections]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: styles.itemGap }}>
      {items.map((item, index) => {
        ...
        const school = edu.institution || edu.school || edu.institute;
