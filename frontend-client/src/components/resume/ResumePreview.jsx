    ...
    <div className="bg-white p-6 rounded-lg shadow-md space-y-6 text-left text-sm">
      ...
      {(fullName || title || email || phone || address || summary || profileImage) && (
        <div>
          ...
          {(email || phone || address) && (
            <p className="text-gray-500">{[email, phone, address].filter(Boolean).join(" | ")}</p>
          )}
        ...
        <div>
          ...
          {education.map((edu, idx) => (
            <div key={idx} className="mb-2">
              {edu.degree && <p className="font-semibold">{edu.degree} in {edu.fieldOfStudy}</p>}
        ...
        <div>
          ...
          {projects.map((project, idx) => (
            <div key={idx} className="mb-2">
              ...
              {project.techStack?.length > 0 && <p>Tech Stack: {project.techStack.join(", ")}</p>}
              {(project.link || project.github) && (
                <div>
                  {project.link && (
                    <a href={project.link} className="text-blue-600 underline" target="_blank" rel="noreferrer">Live</a>
                  )}
                  {project.github && (
                    <>
                      {" | "}
                      <a href={project.github} className="text-blue-600 underline" target="_blank" rel="noreferrer">GitHub</a>
                    </>
                  )}
                </div>
              )}
      ...
      {certifications.some(c => c.name?.trim()) && (
        <div>
          ...
          {certifications.map((cert, idx) => (
            <div key={idx}>
              <p className="font-semibold">{cert.name} - {cert.issuer}</p>
              {cert.date && <p>Issued: {cert.date}</p>}
              {cert.credentialUrl && (
                <a href={cert.credentialUrl} className="text-blue-600 underline" target="_blank" rel="noreferrer">
                  Credential
                </a>
              )}
            </div>
